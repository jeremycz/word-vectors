...
def gradient_analytical(batch: np.array, embeddings: np.array, debug: bool = False) -> np.array:
    """"""
    #
    if len(batch.shape) == 1:
        batch = np.expand_dims(batch, axis=0)
    ...
    # Initialise matrix to store result
    gradient = np.zeros(embeddings.shape, dtype=np.float64)
...
def test_skip_gram_cost_function() -> None:
    ...
    print(f"Cost: {cost(batch0, embeddings, debug=True):.6f}")
    print(f"Expected cost: {2.253856}")

    print()

    batch1 = np.array([[1,0,0], [0,1,0]], dtype=np.int64)
    print(f"Cost: {cost(batch1, embeddings, debug=True):.6f}")
...
def test_skip_gram_gradient() -> None:
    ...
    # Compare analytical gradient with finite-differencing result
    print(gradient_fd(batch0, embeddings, step_size=1e-6))
    print(gradient_analytical(batch0, embeddings, debug=False))

    print()

    batch1 = np.array([[1,0,0], [0,1,0]], dtype=np.int64)
    print(gradient_fd(batch1, embeddings, step_size=1e-6))
    print(gradient_analytical(batch1, embeddings, debug=False))
